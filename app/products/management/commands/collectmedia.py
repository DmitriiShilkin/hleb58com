...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--media-root', default=settings.MEDIA_ROOT, dest='media_root', metavar='DIR',
                            help="Specifies the root directory in which to collect media files.")
        parser.add_argument('-n', '--dry-run', action='store_true', dest='dry_run',
                            help="Do everything except modify the filesystem.")
        parser.add_argument('-d', '--dir', action='append', default=self.media_dirs, dest='media_dirs',
                            metavar='NAME', help="Specifies the name of the media directory to look for in each app.")
        parser.add_argument('-e', '--exclude', action='append', default=self.exclude, dest='exclude',
                            metavar='PATTERNS',
                            help="A space-delimited list of glob-style patterns to ignore. Use multiple times to add more."
                            )
    ...
    def handle(self, *app_labels, **options):
        ...
        # are a list of 2-tuples containing the name of the app providing
    ...
    def process_file(self, source, destination, root, link=False, **options):
        ...
        if not dry_run:
            ...
            try:
                ...
                mode = 0o777  # Default for `os.makedirs` anyway.
                uid = gid = None
            else:
    ...
    def filter_names(self, names, exclude=None, func=fnmatch.filter):
        if exclude is None:
            exclude = []
